...
# with unsymmetric (higher order) thin-plate spline collocation.
#
# Nicholas Kraemer
...
def assemble_collmatrix_tps(PSET, QSET, EXPR_RBF, PDE_PARAM):
	...
	# Collocated polynomial part right
	for i in range(NUM_PTS):
		MTRXX[i, NUM_QTS] = l1 * SPH_00b(PSET[i,0], PSET[i,1], PSET[i,2])
		MTRXX[i, NUM_QTS + 1] = l2 * SPH_10b(PSET[i,0], PSET[i,1], PSET[i,2])
		MTRXX[i, NUM_QTS + 2] = l2 * SPH_11b(PSET[i,0], PSET[i,1], PSET[i,2])
		MTRXX[i, NUM_QTS + 3] = l2 * SPH_12b(PSET[i,0], PSET[i,1], PSET[i,2])
		MTRXX[i, NUM_QTS + 4] = l3 * SPH_20b(PSET[i,0], PSET[i,1], PSET[i,2])
		MTRXX[i, NUM_QTS + 5] = l3 * SPH_21b(PSET[i,0], PSET[i,1], PSET[i,2])
		MTRXX[i, NUM_QTS + 6] = l3 * SPH_22b(PSET[i,0], PSET[i,1], PSET[i,2])
		MTRXX[i, NUM_QTS + 7] = l3 * SPH_23b(PSET[i,0], PSET[i,1], PSET[i,2])
		MTRXX[i, NUM_QTS + 8] = l3 * SPH_24b(PSET[i,0], PSET[i,1], PSET[i,2])
...
if truth == 1:
	...
	if eigentruth == 1:
		...
		plt.semilogy(np.absolute(A1), "8", label = 'K (no precond.)', color = 'k', markersize = 12)
		plt.semilogy(np.absolute(A2), "8", label = 'KP (with precond.)', color = 'r', markersize = 12)
		plt.grid()
		plt.legend()
